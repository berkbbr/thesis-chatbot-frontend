...
export default function Home() {
  ...
  const [isSignedIn, setIsSignedIn] = useState(false) // For Guest/Google login flow

  const messagesEndRef = useRef<HTMLDivElement>(null)

  // Giriş durumunu belirle
  useEffect(() => {
    if (!session) {
  ...
  // --------- GOOGLE SIGNED IN USERNAME OTOMATİK AYARLAMA -----------
  useEffect(() => {
  ...
  // --------- DEVAMINI ESKİ TASARIMINLA BIRAKIYORUM --------
  // Buradan sonrası tamamen eski kodun ile aynı!
  // İstenirse log out (çıkış) için de Google hesabı için signOut() fonksiyonunu kullanabilirsin:

  return (
    <div className="h-screen bg-gradient-to-br from-gray-900 via-slate-900 to-gray-900 flex overflow-hidden">
      {/* ... senin kalan eski kodun burada tamamen aynı devam edecek ... */}
      {/* Logout butonunu: */}
      <button
